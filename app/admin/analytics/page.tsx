...
export default function AdminAnalyticsPage() {
  ...
  useEffect(() => {
    if (status === 'unauthenticated' || (status === 'authenticated' && _session?.user?.userType !== 'employee')) {
      router.push('/login');
    }
  }, [status, _session, router]);
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center gap-6">
            ...
            >
              <motion.div
                className="bg-gradient-to-r from-amber-500 to-amber-600 w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg"
              ...
              <div>
                ...
                <p className="text-slate-400">Detailed insights into banking operations</p>
              </div>
            </motion.div>
          </div>
        </div>
